...
interface FormProps {
  initialData?: Unit;
  onSave: (data: UnitAdd | UnitUpdate) => void;
  onCancel: () => void;
}

export const Form: FC<FormProps> = ({ initialData, onSave, onCancel }) => {
  const { t } = useTranslation();
  const [formData, setFormData] = useState<Partial<Unit>>({ isArchived: false });

  useEffect(() => {
    if (initialData) setFormData(initialData);
  }, [initialData]);

  const handleChange = (field: keyof Unit) => (e: React.ChangeEvent<HTMLInputElement>) => {
