...
import type { Quote, QuoteAdd, QuoteUpdate, QuoteItemAdd } from '../../../backend/shared/types/quote';
...
interface FormProps {
  initialData?: Quote & { items?: any[] };
  onSave: (data: QuoteAdd | QuoteUpdate) => void;
  onCancel: () => void;
}
...
export const Form: FC<FormProps> = ({ initialData, onSave, onCancel }) => {
  ...
  const [formData, setFormData] = useState<Partial<QuoteAdd>>({
    clientId: undefined,
  ...
  const [items, setItems] = useState<Partial<QuoteItemAdd>[]>([]);
  ...
  const handleChange = (field: keyof QuoteAdd) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.type === 'checkbox' ? e.target.checked : e.target.value;
    setFormData((prev) => ({ ...prev, [field]: value }));
  };

  const handleItemChange = (index: number, field: keyof QuoteItemAdd, value: any) => {
  ...
  const previewData: InvoiceTemplateData | null = useMemo(() => {
    ...
    return {
      ...
      invoice: {
        ...formData,
        invoiceNumber: formData.invoiceNumber || 'QT-PREVIEW',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onSave({
      ...
      items: finalItems as QuoteItemAdd[],
      subtotalCents: totals.subtotalCents,
      taxTotalCents: totals.taxTotalCents,
      discountTotalCents: totals.discountTotalCents,
      grandTotalCents: totals.grandTotalCents,
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ width: '100%' }}>
      ...
      {previewData && (
        <Box sx={{ mb: 3 }}>
          <Typography variant="h6" sx={{ mb: 2 }}>{t('common.preview') || 'Live Preview'}</Typography>
